...
use DB;
use Charles\Sms\Sms as SmsClass;
...
trait SmsTestTrait
{
	...
	{
		$key = md5('charles.sms.18988888888');
	...
	public function testSend()
	{
		...
		//3. test use old code.
		$this->app['config']->set('charles.sms.code.maxAttempts', 1);
		$result = Sms::send('18988888888');
		$this->assertTrue($result);
	}
	...
	public function testBadGateway()
	{
		...
		$this->app['config']->set('charles.sms.easy_sms.default.gateways', ['bad_gateway']);

		$sms = new SmsClass(new EasySms(config('charles.sms.easy_sms')), new $storage());
	...
	public function testYunTongXun()
	{
		...
		$gateways   = config('charles.sms.easy_sms.gateways');
		$gateways   = array_merge($gateways, ['yuntongxun' => $yuntongxun]);
		config(['charles.sms.easy_sms.gateways' => $gateways]);

		$storage = config('charles.sms.storage', CacheStorage::class);
		$storage = new $storage();
		$easySms = new EasySms(config('charles.sms.easy_sms'));
